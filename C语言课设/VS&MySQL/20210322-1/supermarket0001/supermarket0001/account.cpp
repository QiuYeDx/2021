...
int registerUser(int initia)
{
    ...
    if (1 == initia)
    {
        printf("\t\t***********您正在注册员工的账号，注册过程中可随时按 # 键后回车以取消注册！*********\n");
        Sleep(600);
        //新注册的帐号默认为员工账号
    ...
    printf("\n\t\t\t\t信息录入成功！\n");
    Sleep(200);
    if (0 == initia) printf("\t\t\t\t您已成功注册，现在返回登录界面...\n");
    else printf("\t\t\t\t正在返回主菜单...\n");
    Sleep(2000);
    system("cls");
    return 1;
...
void changeStatistics_user()
{
    ...
    pNode temp = (pNode)malloc(LEN);
    char choice;
    printf("请选择需要修改的数据类型：1.密码  2.电话号码");
    getchar();
    scanf("%c", &choice);
    ...
    { 
        ...
        while (fread(temp, LEN, 1, fp))
        {
            if (0 == strcmp(id0, temp->id))
            {
                ...
                printf("确定要删除%s这个账号吗？0.取消  1.删除:", temp->id);
                scanf("%d", &flag);
    ...
    while (1)
    {
        ...
        while (fread(temp, LEN, 1, fp))
        {
            ...
            if (0 == strcmp(id0, temp->id))
            {
                ...
                int flag = 0;   //用于用户确认是否删除
                printf("确定要重置%s的密码吗？0.取消  1.确认:", temp->id);
                scanf("%d", &flag);
...
void showStatistics_account()
{
    ...
    printf("\t账号类型\t账号\t      姓名\t  电话号码\t\t性别\t业绩\n");
    while (temp)
    {
        printf("%10s\t%10s\t%10s\t%10s\t%10s\t%.2f\n", temp->idtype, temp->id, temp->name,  temp->phone, temp->sex,temp->achievement);
        temp = temp->next;
    }
    _getch();
    system("cls");
